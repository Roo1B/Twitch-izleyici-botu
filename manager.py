...
class InstanceManager:
    def __init__(
        ...
        logger.info("Yönetici başlangıç")
    ...
    def __del__(self):
        print("Silme yöneticisi: örnekleri temizleme")
    ...
    def spawn_instance_thread(self, target_url=None):
        ...
        if not any([target_url, self.target_url]):
            raise Exception("Hedef hedef url sağlanmadı")
        ...
        with threading.Lock():
            ...
            logger.info(f"{threading.currentThread()} başlangıç örneği yok {browser_instance_id}")
