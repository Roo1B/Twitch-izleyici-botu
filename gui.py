...
class InstanceBox(tk.Frame):
    def __init__(self, manager, parent, *args, **kwargs):
        ...
        self.bind(
            "<Control-1>", lambda event: self.manager.queue_screenshot(instance_id=self.instance_id)
        )  # sol tıklamayı kontrol et
...
class GUI:
    ...
    def spawn_one_func(self):
        print("Bir örnek yumurtlama Lütfen canlı bekleyin ve örneklerin artmasını izleyin.")
    ...
    def spawn_three_func(self):
        print("Üç örnek yumurtlama Lütfen canlı bekleyin ve örneklerin artmasını izleyin.")
        target_url = self.root.children['!entry'].get()
        threading.Thread(target=self.manager.spawn_instances, args=(3, target_url)).start()

    def delete_one_func(self):
        print("Bir örneği yok ediyor Lütfen canlı bekleyin ve örneklerin azalmasını izleyin.")
        threading.Thread(target=self.manager.delete_latest).start()

    def delete_all_func(self):
        print("Tüm örnekleri yok ediyor Lütfen canlı ve izleme örneklerinin azalmasını bekleyin.")
    ...
    def run(self):
        ...
        root.title(f'twitch izleyici botu | v{version} | Root-B')
        ...
        # left
        proxy_available_text = tk.Label(root, text="Kullanılabilir proxy'ler", borderwidth=2)
        proxy_available_text.place(x=40, y=10)
        proxy_available = tk.Label(root, text="0", borderwidth=2, relief="sağlam", width=5)
        proxy_available.place(x=70, y=40)

        lbl_buy = tk.Label(root, text="(buy more)", fg="blue", cursor="hand2")
        lbl_buy.bind("<Button-1>", lambda event: webbrowser.open("https://www.webshare.io/?referral_code=w6nfvip4qp3g"))
        lbl_buy.place(x=58, y=62)

        headless_checkbox = ttk.Checkbutton(
            root, text='headless', command=self.change_headmode, variable=self.headless, onvalue=True, offvalue=False
        )
        headless_checkbox.place(x=55, y=88)

        # right
        instances_text = tk.Label(root, text="Örnekler", borderwidth=2)
        instances_text.place(x=455, y=10)

        alive_instances_text = tk.Label(root, text="canlı", borderwidth=2)
        alive_instances_text.place(x=455, y=40)
        watching_instances_text = tk.Label(root, text="İzleniyor", borderwidth=2)
        watching_instances_text.place(x=455, y=60)

        alive_instances = tk.Label(root, text=0, borderwidth=2, relief="sağlam", width=5)
        alive_instances.place(x=530, y=40)
        watching_instances = tk.Label(root, text=0, borderwidth=2, relief="sağlam", width=5)
        ...
        spawn_one = tk.Button(
            root, width=15, anchor="w", text='1 örnek oluştur', command=lambda: self.spawn_one_func()
        )
        spawn_one.place(x=180, y=40)
        spawn_three = tk.Button(
            root, width=15, anchor="w", text='3 örnek oluştur', command=lambda: self.spawn_three_func()
        )
        spawn_three.place(x=180, y=80)
        destroy_one = tk.Button(
            root, width=15, anchor="w", text='1 örneği yok et', command=lambda: self.delete_one_func()
        )
        destroy_one.place(x=305, y=40)
        destroy_all = tk.Button(
            root, width=15, anchor="w", text='Tüm örnekleri yok et', command=lambda: self.delete_all_func()
        )
        ...
        for row in range(5):
            for col in range(50):
                box = InstanceBox(
                    self.manager, self.root, background='SystemButtonFace', bd=0.5, relief="raised", width=10, height=10
                )
        ...
        # bottom
        lbl = tk.Label(root, text=r"https://github.com/Roo1B/Twitch-izleyici-botu", fg="blue", cursor="hand2")
